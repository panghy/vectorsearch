...
/**
 * Lifecycle smoke test for SegmentBuildWorkerPool: start and close should enqueue sentinels
 * without throwing.
 */
...
class SegmentBuildWorkerPoolTest {
  Database db;
  ...
  void setup() throws Exception {
    ...
    root = db.runAsync(tr -> DirectoryLayer.getDefault()
            .createOrOpen(
                tr,
                List.of("vs-pool", UUID.randomUUID().toString()),
  ...
  @Test
  void start_and_close_pool_enqueues_sentinels() throws Exception {
    ...
    SegmentBuildWorkerPool pool = new SegmentBuildWorkerPool(cfg, dirs, queue);
    pool.start(1);
    pool.close();
    // Best-effort: test passes if no exceptions thrown during start/close lifecycle.
  }
}
