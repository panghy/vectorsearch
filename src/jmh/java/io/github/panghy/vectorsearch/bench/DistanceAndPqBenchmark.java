...
public class DistanceAndPqBenchmark {
  ...
  // ---- Distance state: parameterized by dimension ----
  ...
  // ---- PQ state: fixed dimensions, NOT parameterized ----

  @State(Scope.Thread)
  public static class PqState {
  ...
  // ---- Distance benchmarks (parameterized by dimension) ----

  @Benchmark
  public double l2Distance(DistanceState state) {
    return Distances.l2(state.vecA, state.vecB);
  }

  @Benchmark
  public double cosineDistance(DistanceState state) {
    return Distances.cosine(state.vecA, state.vecB);
  }

  // ---- PQ benchmarks (fixed dimension) ----

  @Benchmark
  public byte[] pqEncode(PqState state) {
    return PqEncoder.encode(state.pqCentroids, state.pqVector);
  }
   ...
   */
  @Benchmark
  public float pqLutDistance(PqState state) {
    float dist = 0f;
    for (int m = 0; m < PqState.PQ_M; m++) {
      dist += state.lut[m][state.pqCodes[m] & 0xFF];
    }
    return dist;
  }
